...
class PLAIN_API Ini {
 ...
 public:
  Ini() noexcept;
  Ini(const char *filename) noexcept;
  ~Ini();
 ...
 public:
  ...
  //Get data.
  sectionset_t *getdata() noexcept;

  //Read string by position;
  const char *getstring(int32_t position) noexcept;

 public:

  template <typename T>
  T get(
    std::string_view section, std::string_view key, T _default = {}) noexcept {
    T r{};
    if (!_get<T>(section.data(), key.data(), r)) return _default;
    return r;
  }
  ...
  bool getstring(
    ...
    const char *_default = "");
  bool getstring_ifexist(
    const char *section, const char *key, char *str, int32_t size);

 private:
  bool _get(const char *section, const char *key, int32_t &result);
  bool _get(const char *section, const char *key, float &result);
  bool _get(
    const char *section, const char *key, std::string &result) noexcept {
    return _getstring(section, key, result.data(), result.capacity() - 1);
  }
  bool _getstring(
