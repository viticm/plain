 ...
 * @date 2021/08/18 10:07
 * @uses The any class(as the std::any in c++17).
 */

#ifndef PF_BASIC_ANY_H_
#define PF_BASIC_ANY_H_

#define any_lite_MAJOR  0
#define any_lite_MINOR  1
#define any_lite_PATCH  0

#define any_lite_VERSION  any_STRINGIFY(any_lite_MAJOR) \
  "." any_STRINGIFY(any_lite_MINOR) "." any_STRINGIFY(any_lite_PATCH)
...
#define  any_USES_STD_ANY ((any_CONFIG_SELECT_ANY == any_ANY_STD) || \
    ((any_CONFIG_SELECT_ANY == any_ANY_DEFAULT) && any_HAVE_STD_ANY))

//
// in_place: code duplicated in any-lite, expected-lite,
// optional-lite, value-ptr-lite, variant-lite:
//

#ifndef pf_basiclite_HAVE_IN_PLACE_TYPES
#define pf_basiclite_HAVE_IN_PLACE_TYPES  1
...
#define pf_basiclite_in_place_t(   T)  std::in_place_t
#define pf_basiclite_in_place_type_t(T)  std::in_place_type_t<T>
#define pf_basiclite_in_place_index_t(K)  std::in_place_index_t<K>

#define pf_basiclite_in_place(   T)  std::in_place_t{}
#define pf_basiclite_in_place_type(T)  std::in_place_type_t<T>{}
#define pf_basiclite_in_place_index(K)  std::in_place_index_t<K>{}
...
#define pf_basiclite_in_place_t(   T)  \
  pf_basic::in_place_t(&)(pf_basic::detail::in_place_type_tag<T>)
#define pf_basiclite_in_place_type_t(T)  \
  pf_basic::in_place_t(&)(pf_basic::detail::in_place_type_tag<T>)
#define pf_basiclite_in_place_index_t(K)  \
  pf_basic::in_place_t(&)(pf_basic::detail::in_place_index_tag<K>)

#define pf_basiclite_in_place(   T)  pf_basic::in_place_type<T>
#define pf_basiclite_in_place_type(T)  pf_basic::in_place_type<T>
#define pf_basiclite_in_place_index(K)  pf_basic::in_place_index<K>

} // namespace pf_basic

#endif // any_CPP17_OR_GREATER
#endif // pf_basiclite_HAVE_IN_PLACE_TYPES
...
class any {
 public:
   ...
   >
   explicit any(pf_basiclite_in_place_type_t(T), Args&&... args)
   ...
   >
   explicit any(pf_basiclite_in_place_type_t(T),
...
inline any make_any(Args&& ...args) {
  return any(pf_basiclite_in_place_type(T), std::forward<Args>(args)...);
}

template< class T, class U, class ...Args >
inline any make_any(std::initializer_list<U> il, Args&& ...args) {
  return any(pf_basiclite_in_place_type(T), il, std::forward<Args>(args)...);
}
...
any_cast(any const * operand) any_noexcept {
  return operand != any_nullptr &&
    operand->type() == typeid(ValueType) ?
    operand->to_ptr<ValueType>() : any_nullptr;
}
