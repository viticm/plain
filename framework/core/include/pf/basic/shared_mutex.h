...
#ifndef PF_BASIC_SHARED_MUTEX_H_
#define PF_BASIC_SHARED_MUTEX_H_
  ...
  void lock(void) {
    for(size_t i = 0; i < N; ++i) {
      ...
      while(true) {
        ...
        if(el_[i].wr_lock.compare_exchange_weak(
              cur_rw_lock, entry_lock::W_MASK))
          break;
      }
    }
  }
    ...
    for(size_t i = 0; i < N; ++i) {
      ...
      while(true) {
        ...
        if(cur_rw_lock != entry_lock::W_MASK)
          throw std::runtime_error(
              "Fatal: unlock but apparently this entry "
              "is shared locked or uninitialized!");
#endif //_RELEASE
...
#endif // PF_BASIC_SHARED_MUTEX_H_
