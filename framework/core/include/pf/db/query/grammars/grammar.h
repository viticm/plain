...
class PF_API Grammar : pf_db::Grammar {
 ...
 public:
   ...
   std::string compile_select(Builder &query);
   ...
   std::string compile_columns(
       Builder &query, const std::vector<std::string> &columns);

   //Compile the "from" portion of the query.
   std::string compile_from(Builder &query, const std::string &table);
   ...
   std::string where_date(Builder &query, const variable_set_t &where);

   //Compile a "where time" clause.
   std::string where_time(Builder &query, const variable_set_t &where);

   //Compile a "where day" clause.
   std::string where_day(Builder &query, const variable_set_t &where);

   //Compile a "where month" clause.
   std::string where_month(Builder &query, const variable_set_t &where);

   //Compile a "where year" clause.
   std::string where_year(Builder &query, const variable_set_t &where);

   //Compile a date based where clause.
   std::string date_based_where(
   ...
   std::string where_exists(Builder &query, const variable_set_t &where);
   ...
   std::string compile_random(const std::string &seed);

   //Compile the "limit" portions of the query.
   std::string compile_limit(Builder &query, int32_t limit);

   //Compile the "offset" portions of the query.
   std::string compile_offset(Builder &query, int32_t offset);

   //Compile the "union" queries attached to the main query.
   std::string compile_unions(Builder &query);

   //Compile a single union statement.
   std::string compile_union(const variable_set_t &unions);

   //Compile an exists statement into SQL.
   std::string compile_exists(Builder &query);

   //Compile an insert statement into SQL.
   std::string compile_insert(Builder &query, const variable_array_t &values);

   //Compile an insert and get ID statement into SQL.
   std::string compile_insert_getid(
       Builder &query, 
       const variable_array_t &values, 
       const std::string &sequence);

   //Compile an update statement into SQL.
   std::string compile_update(Builder &query, const variable_array_t &values);

   //Prepare the bindings for an update statement.
   variable_set_t prepare_bindings_forupdate(
       const variable_set_t &bindings, const variable_array_t &values);

   //Compile a delete statement into SQL.
   std::string compile_delete(Builder &query);

   //Compile a truncate table statement into SQL.
   variable_set_t compile_truncate(Builder &query);

   //Compile the lock into SQL.
   std::string compile_lock(Builder &query, const std::string &value);

   //Determine if the grammar supports savepoints.
   bool supports_savepoints() const;

   //Compile the SQL statement to define a savepoint.
   std::string compile_savepoint(const std::string &name);

   //Compile the SQL statement to execute a savepoint rollback.
   std::string compile_savepoint_rollback(const std::string &name);

   //Concatenate an array of segments, removing empties.
   std::string concatenate(const std::vector<std::string> &segments);

   //Remove the leading boolean from a statement.
   std::string remove_leading_boolean(const std::string &value);
