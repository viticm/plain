...
void plain::tests::test_thread_executor_name() {
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);

  ASSERT_EQ(executor->name_, "thread");
}

void plain::tests::test_thread_executor_shutdown_method_access() {
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
...
void plain::tests::test_thread_executor_shutdown_join() {
...
void plain::tests::test_thread_executor_shutdown_more_than_once() {
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
...
void plain::tests::test_thread_executor_shutdown() {
...
void plain::tests::test_thread_executor_max_concurrency_level() {
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_post_exception() {
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_post_foreign() {
  object_observer observer;
...
void plain::tests::test_thread_executor_post_inline() {
  object_observer observer;
...
void plain::tests::test_thread_executor_submit_foreign() {
  object_observer observer;
  const size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_submit_inline() {
  object_observer observer;
  constexpr size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_bulk_post_foreign() {
  object_observer observer;
  const size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_bulk_post_inline() {
  object_observer observer;
  constexpr size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_bulk_submit_foreign() {
  object_observer observer;
  const size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_bulk_submit_inline() {
  object_observer observer;
  constexpr size_t task_count = 128;
  auto executor = std::make_shared<plain::concurrency::executor::Thread>();
  executor_shutdowner shutdown(executor);
...
void plain::tests::test_thread_executor_bulk_submit() {
  test_thread_executor_bulk_submit_exception();
  test_thread_executor_bulk_post_foreign();
  test_thread_executor_bulk_post_inline();
}

void plain::tests::test_thread_executor_thread_callbacks() {
  test_thread_callbacks(
    [](auto thread_started_callback, auto thread_terminated_callback) {
      return std::make_shared<plain::concurrency::executor::Thread>(
        thread_started_callback, thread_terminated_callback);
    },
    plain::concurrency::executor::detail::make_executor_worker_name("thread"));
}

using namespace plain::tests;

class ThreadExecutor : public testing::Test {
...
TEST_F(ThreadExecutor, testShutdown) {
  test_thread_executor_shutdown();
}

TEST_F(ThreadExecutor, testName) {
  test_thread_executor_name();
}

TEST_F(ThreadExecutor, testMaxConcurrencyLevel) {
  test_thread_executor_max_concurrency_level();
}

TEST_F(ThreadExecutor, testPost) {
  test_thread_executor_post();
}

TEST_F(ThreadExecutor, testSubmit) {
  test_thread_executor_submit();
}

TEST_F(ThreadExecutor, testBulkPost) {
  test_thread_executor_bulk_post();
}

TEST_F(ThreadExecutor, testBulkSubmit) {
  test_thread_executor_bulk_submit();
}

TEST_F(ThreadExecutor, testCallbaks) {
